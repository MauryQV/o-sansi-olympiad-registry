...
const ModalNuevaArea = ({ mostrar, cerrar, agregarArea, areaAEditar }) => {
  ...
  useEffect(() => {
    ...
    if (areaAEditar) {
      setNombre(areaAEditar.nombre || areaAEditar.nombre_area || "");
      setDescripcion(
        areaAEditar.descripcion || areaAEditar.descripcion_area || ""
      );
      setCosto(
        areaAEditar.costo !== undefined ? areaAEditar.costo.toString() : ""
      );
    } else {
      setNombre("");
      setDescripcion("");
  ...
  const validarCampos = () => {
    ...
    if (!nombre.trim()) {
      ...
      nuevosErrores.nombre =
        "Solo se permiten letras y espacios (3-50 caracteres).";
    }
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        <div className="modal-header">
          <h3>{areaAEditar ? "Editar Área" : "Nueva Área"}</h3>
          <button
            type="button"
          ...
          <form onSubmit={handleSubmit}>
            <label>
              ...
              <input
                ...
                placeholder="Nombre del área científica"
                minLength={3}
                maxLength={50}
                required
            ...
            <label>
              ...
              />
              {errores.costo && (
                <span className="error-text">{errores.costo}</span>
              )}
            </label>
